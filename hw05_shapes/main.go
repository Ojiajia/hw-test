...
func main() {
	...
	/*
		ChoosedVar.Choice = 2 // площадь треугольника
		ChoosedVar.CalcArea(SomeVar)
		ChoosedVar.Choice = 3 // площадь прямугольника
		ChoosedVar.CalcArea(SomeVar)
	*/
	// print("circle area = ", Scircle.CalcArea())
	// print("triangle area = ", Striangle.CalcArea())
	// print("rectangle area = ", Srectangle.CalcArea())

	// s, ok := varShape.(float32) // если не это то пизда
